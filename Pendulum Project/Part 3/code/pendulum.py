...
def analyze_file(filename):
    ...
    chi2r_curve_fit_decay = 0
    if True:
        reverse_measured_angles_radians = np.abs(np.flip(measured_angles_radians))
        ...
        # calculate chi2r using the fit_samples
        chi2r_curve_fit_decay = utils.chi2reduced(measured_decay_angles[:2000], 
                  model_function_only_decay(measured_times[:2000], 
                                 init_angle_radians,
                                 period,  
                                 decay_constant,
                                 init_phase_radians),
    ...
    label = r"$\theta(t) = \theta_0 e^{-t/\tau}$" % chi2r_curve_fit_decay
    ...
    a0.set_title("Angle vs Time (M = %.f \u00b1 1 grams, L = %.1f \
\u00b1 0.5 cm)" % \
    ...
    a1.plot(measured_times, (measured_angles_radians-predicted_angles))

    ylim = np.max(measured_angles_radians[:100])
    a0.set_ylim([-ylim * 1.2, ylim * 2.5])
